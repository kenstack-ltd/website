...
?>
        ...
        if ((!function_exists('has_site_icon') || !has_site_icon()) && !empty($bridge_qode_options['favicon_image'])) { ?>
            <link rel="shortcut icon" type="image/x-icon" href="<?php echo str_replace('http://', 'https://', esc_url($bridge_qode_options['favicon_image'])); ?>">
            <link rel="apple-touch-icon" href="<?php echo str_replace('http://', 'https://', esc_url($bridge_qode_options['favicon_image'])); ?>" />
        <?php }
    }
