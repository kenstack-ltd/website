    ...
    if(isset($bridge_qode_options["post_types_names_$post_type"])) {
        if($bridge_qode_options["post_types_names_$post_type"] == $post_type) {
            ...
            if($bridge_qode_options['enable_twitter_share'] == "yes") {
                ...
                $html .= '<a href="#" onclick="popUp=window.open(\'https://twitter.com/intent?text=' . urlencode(bridge_qode_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;">';
